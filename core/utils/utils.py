...
def forward_interpolate(flow):
    ...
    ht, wd = dx.shape
    x0, y0 = cp.meshgrid(cp.arange(wd), cp.arange(ht))
    ...
    flow = cp.stack([flow_x, flow_y], axis=0)
...
def preprocess_image(image, device):
    ...
    img = torch.from_numpy(image_np).permute(2, 0, 1).float()
    return img[None].to(device)

def generate_flow_output(flo):
    """
    ...
    """
    flo = cp.asarray(flo[0].permute(1,2,0))
    flo = flow_to_image(flo)
    return flo[:, :, [2,1,0]].get()